...
impl Mapper for UNRomMapper {
	fn map(&self, address: u32) -> u32 {
		let bank = match address < 0xC000 {
			true => match self.program_bank_num {
				0 => 0,
				n => self.register.load() % n
			},
			false => self.program_bank_num.saturating_sub(1)
		} as u32;
...
mod tests_nrom_mapper {
	...
	#[test]
	fn unrom_wraps_selected_program_bank_to_available_banks() {
		let mut m = UNRomMapper{
			program_bank_num: 8,
			register: Register::<u8>::new()
		};
		m.store(0x8000, 0x0A);
		assert_eq!(0x8000, m.map(0x8000));
		assert_eq!(0xBFFF, m.map(0xBFFF));
		assert_eq!(0x1C000, m.map(0xC000));
		assert_eq!(0x1FFFF, m.map(0xFFFF));
	}
}
